...
namespace PACZU{
	...
	struct entry {
		...
		using entry_t_ref_wrapper_v = std::tuple<key_t, std::reference_wrapper<val_t>>;
		using filling_curve_t = void;
		using key_entry_pointer = std::pair<key_t, entry_t*>;

		static inline key_t const& get_key(entry_t const& e) { return std::get<0>(e); }
		static inline val_t const& get_val(entry_t const& e) { return std::get<1>(e); }
		static inline void set_val(entry_t& e, val_t const& v) { std::get<1>(e) = v; }
		static inline entry_t to_entry(key_t const& k, val_t const& v) { return std::make_tuple(k, v); }
		static inline aug_t from_entry(entry_t const& e) { return from_entry(get_key(e), get_val(e)); }
		static inline aug_t from_entry_array(entry_t* et, size_t sz) {
			aug_t a = get_empty();
			for(size_t i=0; i<sz; i++) a = combine(a, from_entry(et[i]));
			return a;
		}
	...
	auto size_in_bytes(){
			...
			size_t internal_nodes_space = sizeof(typename zmap::gc_type::regular_node) * inte_used;
		auto [used, unused] = parlay::internal::get_default_allocator().stats();
		return make_tuple(internal_nodes_space, used);
	}

	auto print_stats(){
			zmap::gc_type::print_stats();
	}
