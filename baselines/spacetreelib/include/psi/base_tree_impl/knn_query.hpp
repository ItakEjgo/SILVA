...
template <typename Traits, typename DerivedTree>
template <typename leaf_type, typename Range, typename Filter>
void base_tree<Traits, DerivedTree>::knn_leaf(
	node *T, point_type const &q, bounded_queue<point_type, Range> &bq, Filter filter)
{
	assert(T->is_leaf);
...
template <typename Traits, typename DerivedTree>
template <typename leaf_type, is_binary_node interior_type, typename Range, typename Filter>
void base_tree<Traits, DerivedTree>::knn_binary(
	node *T, point_type const &q, bounded_queue<point_type, Range> &bq,
	box_type const &node_box, knn_logger &logger, Filter filter)
		...
		typename base_tree<Traits, DerivedTree>::hyper_plane_type>
{
	if (T->is_leaf) {
		logger.vis_leaf_num++;
		knn_leaf<leaf_type>(T, q, bq, filter);
		return;
	}
	...
	knn_binary<leaf_type, interior_type>(go_left ? ti->right : ti->left, q,
					     bq, box_cut.get_box(), logger, filter);
	return;
}
...
void base_tree<Traits, DerivedTree>::knn_binary_box(
	node *T, point_type const &q, bounded_queue<point_type, Range> &bq,
	knn_logger &logger, Filter filter)
{
	if (bq.size() &&
...
template <typename Traits, typename DerivedTree>
template <typename leaf_type, is_multi_node interior_type, typename Range, typename Filter>
...
void base_tree<Traits, DerivedTree>::knn_multi_expand_box(
	node *T, point_type const &q, dims_type dim, bucket_type idx,
	bounded_queue<point_type, Range> &bq, knn_logger &logger, Filter filter)
{
	if (T->size == 0) {
		return;
	}
	...
	knn_logger &logger, Filter filter)
{
	...
	for (bucket_type i = 1; i < interior_type::get_regions(); ++i) {
		...
		knn_multi<leaf_type, interior_type>(
			ti->tree_nodes[dists[i].second], q, bq, logger, filter);
	}
