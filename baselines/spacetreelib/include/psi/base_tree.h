...
namespace psi
{
struct default_filter { template<typename pt> bool operator()(const pt& p) const { return true; } };
...
public:
	...
	template <typename leaf_type, typename Range, typename Filter = default_filter>
	static void knn_leaf(node *T, point_type const &q,
			     bounded_queue<point_type, Range> &bq, Filter filter = Filter());

	/* search knn in the binary node */
	template <typename leaf_type, is_binary_node interior_type,
		  typename Range, typename Filter = default_filter>
	static void knn_binary(node *T, point_type const &q,
			       bounded_queue<point_type, Range> &bq,
			       box_type const &node_box, knn_logger &logger, Filter filter = Filter())
	...
	static void knn_binary_box(node *T, point_type const &q,
				   bounded_queue<point_type, Range> &bq,
				   knn_logger &logger, Filter filter = Filter());
