...
void ParallelKDtree<point>::k_nearest(node* T, const point& q, const dim_type DIM, kBoundedQueue<point, StoreType>& bq,
    ...
    k_nearest(go_left() ? TI->left : TI->right, q, DIM, bq, firstBox, visNodeNum, filter);
    if (Num::Gt(p2b_min_distance(q, secondBox, DIM), bq.top_value()) && bq.full()) {
        return;
    }
    k_nearest(go_left() ? TI->right : TI->left, q, DIM, bq, secondBox, visNodeNum, filter);
    return;
}
